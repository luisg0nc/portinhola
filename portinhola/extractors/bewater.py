...
# Table rows end with amount (4 decimals) + VAT code, optionally preceded by a
# unit price (6 decimals). The quantity ("Faturado") column always precedes.
LINE_RE = re.compile(
    r"^(?P<desc>.+?)\s+"
    r"(?P<qty>-?\d+,\d{4})\s+"
    r"(?:(?P<price>-?\d+,\d{6})\s+)?"
...
# (2) água 6%, (3) saneamento 6%, (23) não sujeito a IVA (TRSU/municipal)
VAT_CODES: dict[str, int | None] = {"2": 6, "3": 6, "23": None}
